...
const sendOtp=async(req,res,next)=>{
    try {
        ...
        if(!userData){
            ...
            const subject= "ElectronDigital: Verify your email"
            const content=`Your OTP is ${otp}. Expire in 10 minutes`
            await mailer.sendEmail(email,subject,content)
            res.render('auth/email-otp-auth',{layout:'auth-layout',title:'ElectronDigital|OTP'})
